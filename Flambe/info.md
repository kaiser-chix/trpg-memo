卓のメモは[こちら](/Flambe/memo.md)

# ◆参加者
- GM
  - りありす
- PL
  - アルト
    - [ロボジャック](https://trpg.x0.com/ytsheet2/sw2.5/?id=6HR4Mv)
  - かいざー
    - [オズ](https://trpg.x0.com/ytsheet2/sw2.5/?id=oapHdG)
  - shilvarion
    - [フィルティニア・シュヴァンツ](https://trpg.x0.com/ytsheet2/sw2.5/?id=sWuCS3)
  - 46usagi
    - [セルフィア・ルビアストス](https://trpg.x0.com/ytsheet2/sw2.5/?id=0g0iwY)
  - Noa
    - [イオ・ライネス](https://trpg.x0.com/ytsheet2/sw2.5/?id=YPOGJQ)
  - むむがみ
    - [アル＝ヴァシュタール](https://trpg.x0.com/ytsheet2/sw2.5/?id=L3HCGV)
  - リューズ
    - [ルーナ → ルーナリット・シュヴァンツ](https://trpg.x0.com/ytsheet2/sw2.5/?id=TkqWa8)

# ◆開催場所
[ココフォリア](https://ccfolia.com/rooms/a8o2Ua7lR)

# ◆日程調整
[調整さん](https://chouseisan.com/s?h=3c2ce3ebacd44435a64b1430a33bbc66)

# ◆登場したNPC
## ▼ “車長”アル
### エルフ／男性／？歳
大食堂列車のオーナーです。<br>
その実力は未知数ですがその興味と判断基準は全てが『食』であり、そこに蛮族と人族の区別はありません。<br>
そのため彼の列車は蛮族も人族も、等しく乗客・乗員、そして食材として扱われるとかなんとか<br>
また、大陸のどこかには彼の最高の食材を隠し集めた、食料庫があると噂されていますが見つけた人はいません。<br>
たまに自動操縦にして抜け出しては怒られています。<br>


## ▼ “総料理長”
### リカント／女性／？歳
大食堂列車において、総料理長を務めるリカントの女性です。<br>
厨房全体の統括が仕事だが、それ以上にアルの食事を作る事が1番の仕事。<br>
自身の料理人としての全盛期が今であり、そしてこれからは衰退するのが見え始め、後進の育成に苦慮しています。<br>
ちなみに怒らせたらとても怖いそうです。<br>


## ▼ “車掌長”ペティ
### コボルド／男性／20歳
大食堂列車において、車掌長を務めるコボルトの男性です。<br>
多様な言語に通じ、ほぼ全ての種族の存在と会話ができます。<br>
客への対応や、言語が通じない同士の通訳などその業務は多岐に渡り、常に忙しそうにしていますが話しかけると微笑みを絶やさずに対応してくれるでしょう。<br>

## ▼ ガミルド
### リルドラケン/男性/？
イオの友人の友人(自称)<br>
友人にお願いされてフランベにイオを誘った張本人<br>
会うたびに尻尾を自慢してくる<br>
気のいいお兄ちゃんみたいな感じの人です。<br>
割とふらふらしている印象ですが<br>
どこで何をしているかわからないような人でもあります。<br>
本人曰くめっちゃ強いとのことだが…？<br>

## ▼ カイザードローン零式【ディッシュ】
### 魔動機／不明／不詳
竜工廠列車（ドラコ＝ファクトレイン）の車長 兼 防衛機構のカイザードローン零式<br>
ディッシュと呼ばれています。<br>
とあるハイマンが元からあった魔動機文明の遺産を元に改造を施した遺物です<br>
本体は竜工廠列車に内蔵されている巨大なマギスフィアに内蔵された AI です<br>
飛行距離は列車から 100 m の範囲内であれば移動ができます。<br>
それ以上飛行することも可能ですが、機能が落ちます。<br>
制作者のことをマスターと呼んでおり、製作者の命令により列車を稼働させています。<br>
わりと陽気な性格をしていますが、製作者の意向らしいです。<br>

# ◆【料理長による料理技能成長ルール】
## ▼ 判定タイミング
* シナリオの **開始時または終了時** に、料理長の「コック技能」が成長できるかを判定します。
* この判定は、**シナリオ中に1回だけ** 行えます。

## ▼ 判定方法
以下の3つの項目について判定を行います。
1. **材料の目利き判定**：コック技能 ＋ 知力
2. **料理判定**　　　　：コック技能 ＋ 器用度
3. **盛り付け判定**　　：コック技能 ＋ 生命力

## ▼ 専用成長ポイントの蓄積
* 各判定で**成功した場合**は、判定の合計値の**半分**を「コック専用成長P」として貯めます。
* **失敗した場合**は、判定の合計値をそのまま「コック専用成長P」として貯めます。

## ▼ 成長の条件
* 貯めた「コック専用成長P」が、次のレベルに必要なポイントを超えた場合は、通常の成長とは別に「コック技能」を成長させることができます。
* 成長に必要なポイントは、
  **コック技能の現在レベル × 50P** です。
  * **例：**
    * 1Lv → 2Lv に上げるには **50P** が必要
    * 2Lv → 3Lv に上げるには **100P** が必要

## ▼ その他
* 成功した際の追加報酬などの詳細は、別途設定します。

# ◆【事業システム】
## ▼ 列車内への出店
- **収入発生の条件**
  - 店舗の収入は、以下のすべての条件を満たした場合に発生します。
    - GM（ゲームマスター）が定めた一定時間が経過していること。
    - 店舗に「販売可能な特定のアイテム」が存在していること。
    - 店舗に「店員」が存在していること。
- **アイテムと素材のランク**
  - 制作されたアイテム、および制作に使用する素材には「ランク」が設定されている。
  - 高ランクの素材を使用すると、高ランクのアイテムを制作しやすくなる。
  - **ランクの一覧**
    - Dランク：粗悪品
    - Cランク：安価品
    - Bランク：通常品
    - Aランク：優良品
    - Sランク：希少品
    - SSランク：伝説品
    - ???：神話品
- **売却におけるランク・技能の影響**
  - アイテムのランクや、一般技能のレベルによって、アイテムが売れる速さと売値が変動する。

## ▼ 列車内への工房の設置
- **購入義務**
  - 各「一般技能」ごとに、列車内でアイテムの製作等をする場合は、対応する道具を必ず 1 つ購入しなければならない。

- **ランクの進行**
  - 道具は必ず **B ランク** から使用を開始する。
  - 上位ランクを使用するには、B ランク → A ランク → S ランク → SS ランク の順に、1 段階ずつランクを上げていく必要がある。

- **ランクごとの性能と価格**

    | ランク  | 品質   | 効果                          | 時短効果             | 価格       |
    |---------|--------|-------------------------------|----------------------|------------|
    | B ランク | 通常品 | 加工技能 +1                   | なし                 | 1,000 G    |
    | A ランク | 優良品 | 加工技能 +2                   | なし                 | 5,000 G    |
    | S ランク | 希少品 | 加工技能 +4                   | 制作時間短縮 1 ～ 7 日  | 25,000 G   |
    | SS ランク| 伝説品 | 加工技能 +8                   | 制作時間短縮 1 ～ 4 週  | 100,000 G  |


## ▼ 自分の事業への投資
- **投資を行うことで、PC（プレイヤーキャラクター）は以下のボーナスを得られる可能性がある**
  - 素材採取判定へのボーナス
  - 商品売上へのボーナス
  - など……
- 投資は **1,000 G 単位** で行う必要がある


## ▼ 名誉点をブランドへ変換
- **名誉点の消費とブランドランク**
  - 名誉点の消費は、『Ⅱ』⇒ 137 頁に記載されている「冒険者ランク」の規定に従う
    - ただし、効果を得るためには **パーティ全員が同じ名誉点を消費する必要がある**
  - 消費した名誉点に応じて、売上に倍率を掛けることで「商品のブランド価値」を表す
    - 冒険者ランクと混同しないように、名誉点による知名度は **ブランドランク** と呼称する
- **ブランドランクの例**
  - [ダガー] ブランド：局所的に知られるブランド → 売上倍率 **1.1倍**
  - [レイピア] ブランド：界隈の新参ブランド → 売上倍率 **未定**
  - [ブロードソード] ブランド：庶民から冒険者まで広く知れ渡ったブランド → 売上倍率 **未定**
  - など……

## ▼ 購買部員シオンとのコネクション
- シナリオ間で"購買部員シオン"とのコネクションレベルを上昇させた場合、そのシナリオ間のみバイバイバリューでの購入を許可する
